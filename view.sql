...
FROM "wb.dbt::pack.reqheader" as r
...
SELECT w.*, dr."fio",
       ...
       (SELECT COUNT (*) FROM "wb.dbt::pack.reqheader" as r WHERE r."waybill_id" = w."id") AS "req_cnt",
       (SELECT COUNT (*) FROM "wb.dbt::pack.schedule" as s WHERE s."waybill_id" = w."id") AS "sch_cnt",
       (SELECT COUNT (*) FROM "wb.dbt::pack.reqhistory" as h WHERE h."waybill_id" = w."id") AS "hist_cnt",
       (SELECT COUNT (*) FROM "wb.dbt::pack.gasspent" as g WHERE g."waybill_id" = w."id") AS "gas_cnt"
FROM "wb.dbt::pack.waybill" as w
         LEFT OUTER JOIN "wb.dbt::pack.driver" as dr ON w."bukrs" = dr."bukrs" AND w."driver" = dr."pernr"
         LEFT OUTER JOIN "wb.dbt::pack.equipment" as e ON w."equnr" = e."equnr";
...
FROM "wb.dbt::pack.reqheader" as r
         LEFT OUTER JOIN "wb.dbt::pack.waybill" as w ON w."id" = r."waybill_id"
         LEFT OUTER JOIN "wb.dbt::pack.statustext" as s ON s."id" = r."statusreason";


CREATE VIEW "v_driver" AS
SELECT dr.*
FROM "wb.dbt::pack.driver" as dr;
...
SELECT
    ...
    CASE
        ...
        END as "pttype_ru"
FROM "wb.dbt::pack.gasspent" as g
         LEFT OUTER JOIN "wb.dbt::pack.waybill" as w ON w."id" = g."waybill_id"
         LEFT OUTER JOIN "wb.dbt::pack.equipment" as e ON w."equnr" = e."equnr"
         LEFT OUTER JOIN "wb.dbt::pack.gastype" as t ON g."gasmatnr" = t."matnr"
